...
export async function loadArabicFont(
  ...
  try {
    ...
    if (!fontConfig) {
      console.warn(`Font ${fontName} not found, using Vazirmatn`);
      const vazirmFont = AVAILABLE_ARABIC_FONTS.find((f) => f.value === "Vazirmatn");
      if (!vazirmFont) return "helvetica";
      fontName = "Vazirmatn";
    }
